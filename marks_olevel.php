...
<div id="page-wrapper">
	...
	<!-- /.this contains the form section for selecting marks specifications-->		
     ...
     <form role="form" method="post"  name='form1'  action="" ">
		...
		<div class="col-xs-2">
			<label for="exampleInputEmail1">Subject Teacher</label>
			  <select name="teachername"  class="form-control" >
				<?php 
				//sql query code goes here
				$query=mysql_query('SELECT * FROM teachers ORDER BY name ASC') or die(mysql_error());
				while ($row=mysql_fetch_array($query)){
					$teachername=$row['name'];
				?>	
					<option><?php echo $teachername; ?></option>
				<?php						
				}			
				?>
				</select>
...
if (isset($_POST['cmdsearch'])){
	...
	?>
	<div class = "box-header with-border">			
		<h4 style="text-align:right;background-color:blue;"><br>
...
if(isset($_POST['submit_marks'])){
	...
	$teacher = $_POST['teacher'];//class subject teacher
	...
	$dbstatus = mysql_query("SELECT * FROM $marks_table WHERE term='$term' AND class='$class' AND type='$exam_type'
	...
	if(mysql_num_rows($dbstatus)==0){//if number of students from db is zero
		...
		for ($i=0;$i<$nof_items;$i++){//for each item in the array of items
			...
			$insert_marks=mysql_query("INSERT INTO $marks_table
			(sno,term,year,type,class,$sub_abbr) 
			VALUES ('$std_numbers[$i]','$term','$Year','$exam_type','$class','$std_mark')",$con) or die(mysql_error());
		}
	...
	if(mysql_num_rows($dbstatus)!=0){//if there are some students in the database
		...
		for ($i=0;$i<$nof_items;$i++){//for each item in the array of items.
			...
			$exists = mysql_query("SELECT * FROM $marks_table WHERE sno='$std_numbers[$i]' AND term='$term' 
			...
			if(mysql_num_rows($exists)==0){//if student number is not in marks table
				...
				$insert_marks=mysql_query("INSERT INTO $marks_table (sno,term,year,type,class,`$sub_abbr`) 
				VALUES ('$std_numbers[$i]','$term','$Year','$exam_type','$class','$std_mark')",$con) or die('Hoooo: '.mysql_error());
			}
			if(mysql_num_rows($exists)>0){//if student number exists atleast once in the marks table
				...
				AND class='$class' AND type='$exam_type' AND year='$Year'",$con) or die('Haaaa: '.mysql_error());
			}
		}
			echo "<script> alert ('";
			echo $std_mark_counter." Records Saved Successfully!";
