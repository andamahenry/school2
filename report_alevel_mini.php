...
<div class="empty">
	...
	<div class="box-header with-border">
		<h3 class="box-title">Select Appropriate Fields to To View A-Level Mini-Report</h3><hr>
    </div>
...
global $term,$class,$report_type,$year,$stream;
	...
	function aggFunction($avg_mks){//function that calculates aggregates
		...
		$query_1=mysql_query("select * FROM paper_grades_alevel ",$con)or die(mysql_error());
		...
		if($avg_mks<0 OR $avg_mks>100){
			$grade = 'x' AND $point = 'x' AND $comment = 'Missed' ;;
		}
	...
	function subjectTeacher($subj_name,$class){//to change this function after creating a table that includes subject papers also
		require 'connect.php';
	...
	if(strtolower($report_type)=='b.o.t'){
		$marks_table = "marks_alevel_bot" AND $requirements = '';
		$next_term_begins_on = '';
	}
	if(strtolower($report_type)=='mid'){
		$marks_table = "marks_alevel_mot" AND $requirements = '';
		$next_term_begins_on = '';
	}
	if(strtolower($report_type)=='e.o.t'){
		$marks_table = "marks_alevel_eot" AND $requirements = 'REQUIREMENTS: Refer To The Circular.';
		$next_term_begins_on = "NEXT TERM BEGINS ON: ".$next_term;
	}
	
	//Query for student names from database
	$student_names=mysql_query("SELECT COUNT(sno) AS ttotal FROM students WHERE room_name='$class' AND stream='$stream' AND active='yes'",$con) or die(mysql_error());
	while($row=mysql_fetch_array($student_names)){
		$ttotal = $row['ttotal'];
	}
				...
				<div  style="background-image:url('*logo/logo.jpg');background-repeat:no-repeat;
					...
					<div style="z-index=100;transparency:100%">
						<table class="tstyle" onload="myFunction(<?php echo $sno;?>)">
							<thead>
								<th>SUBJECT</th><th>PAPER</th><th><?php echo $report_type.' (%)';?></th><th>GRADE</th>
								<th>SUBJECT <br>GRADE</th><th>POINTS</th><th>COMMENT</th><th>INITIALS</th>
							</thead>
							<tbody>
								<?php 
								$ss_counter=0;//Initialize subsidiary pass counter
								$_gp = mysql_query("SELECT gp FROM $marks_table WHERE sno='$sno' AND term='$term' AND type='$report_type' AND 
								year='$Year' AND class='$class' AND gp != 'NULL' ",$con) or die(mysql_error());
								if(mysql_num_rows($_gp)!=0){//Stop script if no records exist in the database									
									while($row=mysql_fetch_array($_gp)){$gp_mark1=$row['gp'];}
									$gp_mark = markSanitizer($gp_mark1);//clean marks before output to user
									$grade = aggFunction($gp_mark1);//Get paper grade
								...
								for($i=0;$i<COUNT($subjects);$i++){
									...
									$paper_mark_array = array();//array for each paper and mark
									$js_points_array = array();//array for each paper points
									//Draw Table rows for each subject
									$_getmarks=mysql_query("SELECT paper,$abbreviations[$i] FROM $marks_table WHERE sno='$sno' AND class='$class' AND year='$Year' AND 
										term='$term' AND type='$report_type' AND $abbreviations[$i] != 'NULL' AND $abbreviations[$i] != -2 ORDER BY paper ASC",$con) or die(mysql_error());
									while($row=mysql_fetch_array($_getmarks)){
										$paper = $row['paper'];
										$mark = $row[$abbreviations[$i]];
										$paper_mark_array[$paper] = $mark;		
										//array_push($js_mark_array,$mark);
										array_push($valid_subjects_array,$abbreviations);		
									}
									if(COUNT($valid_subjects_array)==0){continue;}//Skip to next subject if the learner has invalid marks
									
									$j=0;$k=0;$row_span_counter = COUNT($paper_mark_array);$this_subject = $codes[$i].'&nbsp'.ucwords($subjects[$i]);
									$attribute = '';$total_counter=0;
									foreach($paper_mark_array as $paper => $mark){
											array_push($js_points_array,aggFunction($mark)[1]);
										
									}
									foreach($paper_mark_array as $paper => $mark){
										if($j==0 && $k==0){
											$row_span_counter = $row_span_counter = COUNT($paper_mark_array);
									...
									?>
										<tr>
											<td style="text-align:left;" rowspan="<?php echo $row_span_counter;?>" <?php echo $attribute;?> >
													<b>&nbsp;&nbsp;&nbsp;&nbsp;<?php echo ucwords($this_subject);?>
											</td>
											<!--/td-->
											<td><?php echo $paper;?></td><td><?php echo markSanitizer($mark);?></td>
										...
										//$js_id = $sno;
										if($total_counter==COUNT($paper_mark_array)){
								...
								foreach($subsidiaries as $scode => $sname){
									$query1=mysql_query("SELECT paper,$sname FROM $marks_table WHERE sno='$sno' AND class='$class' AND year='$Year' AND term='$term' AND 
										type='$report_type' AND $sname != 'NULL' AND $sname != -2 ",$con) or die(mysql_error());
									if(mysql_num_rows($query1)==0){continue;}//skip subsidiary if it has no valid marks in the db table
									
									//print mysql_num_rows($query1).'==>'.$scode.'==>'.$sname.'<br>';
									while($row1 = mysql_fetch_array($query1)){//Extract paper and mark and draw appropriate table rows with data
										$s_paper = $row1['paper'];//paper number
										$s_mark = $row1[$sname];//paper mark
										?>
										<tr>
											<td style="text-align:left;">&nbsp;&nbsp;&nbsp;&nbsp;<b><?php echo $scode.'&nbsp;'.ucwords($sname);?></b></td><td><?php echo $s_paper;?></td>
											<td><?php echo markSanitizer($s_mark);?></td><td><?php echo aggFunction($s_mark)[0];?></td>
											<?php 
											...
											<td><?php echo subjectTeacher($s_subject[$s_abbr],$class);?></td>
										</tr>
										
										<?php
									}//End of extraction of optional subsidiary paper details
								...
								<td></td><td></td><td>".markSanitizer($total_score)."</td><td></td><td></td></tr>";
								?>				
							</tbody>							
						</table><br>
							<div class="row">
						...
						<div>
							<div class="row">
								<div class="col-xs-3"><label>&nbsp;Class Teacher's Comment: &nbsp;</label></div>					
