...
$id = $_GET['id'];
$query = mysql_query("SELECT * FROM teachers WHERE id='$id'",$con) or die(mysql_error());
while($row = mysql_fetch_array($query)){
	$sno=$row['sno']; $name=$row['name'];$gender=$row['gender'];$address=$row['address'];$phone=$row['phone'];
	$username=$row['username'];$age=$row['age'];$email=$row['email'];$account=$row['account'];$initials=$row['initials'];
}
...
if (isset($_POST['update'])){
	...
	// if no error occured, continue Updates....
		$query = mysql_query("UPDATE teachers SET sno='$sno',name='$name',gender='$gender',address='$address',phone='$phone',
		username='$username',age='$age',email='$email',account='$account',initials='$initials' WHERE id='$id'",$con)or die(mysql_error());
		
?>
<script>
alert('Succsessfully Saved');
window.location = "teacher_view.php";
</script>
<?php
}
?>
			...
			<div>
				...
				<div>
					<div class="form-group">
							  ...
							  <div class="col-md-3">
					<input type="text" name="name" id = "name" value="<?php echo $name;?>" class="form-control input-md" placeholder="Enter Full Name" required/> 
						</div>
					<div class="col-md-3">
					<input type="text" class="form-control" value="<?php echo $sno;?>" name="student_number" id="sno" placeholder="Serial Number" readonly></input>
					</div>	
					</div>
				<script> 
				...
				<div class="form-group">
						  ...
						  <div class="col-md-3">
				<input type="text" name="initials" id = "initials" value="<?php echo $initials;?>" class="form-control input-md" placeholder="Teacher's Initials" required/> 
				</div>
				<div class="col-md-3">
					<input type="text" class="form-control" name="account_number" id="accno" value="<?php echo $account;?>" placeholder="Payment A/C number" />
				</div>
				</div>
				...
				<div class="form-group">
				  ...
				  <div class="col-md-3">
						<input type="number" name="age" id = "age" value="<?php echo $age;?>" class="form-control input-md" title="input number only" required/>
					</div>
				</div>
				<div class="form-group">
						<label class="col-md-5 control-label" for="rental">E-mail:</label>
						<div class="col-md-3">
						<input type="text" name="email" id = "email" value="<?php echo $email;?>" title="Enter E-mail address" class="form-control input-md" placeholder="Enter E-mail address" required/>
					</div>
				</div>
				<div class="form-group">
					<label class="col-md-5 control-label" for="rental">Address:</label>
					<div class="col-md-3">
						<input type="text" name="address" id = "address" value="<?php echo $address;?>" class="form-control input-md" placeholder="Address" required/>
					</div>
				</div>
				<div class="form-group">
					<label class="col-md-5 control-label" for="rental">Contact No.:</label>
					<div class="col-md-3">
						<input type="text" name="contact" id = "contact" value="<?php echo $phone;?>" class="form-control input-md" title="input number only" placeholder="Contact Number" required/>
					</div>
				</div>
				<!--div class="form-group">
				...
				<button type="submit" id="submit" name="update" class="btn btn-success">Save</button>
