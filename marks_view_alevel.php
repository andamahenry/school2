...
<div id="page-wrapper">
	...
	<div class="box-header with-border">
		<h3 class="box-title">Select Appropriate Fields to View/Edit A-Level Marks</h3><hr>
    </div>
     ...
     <form role="form" method="post"  name='form1'  action="" ">
	   ...
	   <div class="col-xs-2">
		   <label for="exampleInputEmail1">Class</label>
			...
			<?php 
				$query=mysql_query('SELECT * FROM classes WHERE abbr >= 5 ORDER BY name ASC') or die(mysql_error());
		...
		<div class="col-xs-2">
			<label for="exampleInputEmail1">Subject</label>
				...
				while ($row=mysql_fetch_array($query)){
					$name=$row['subject'];$sub_abbr = $row['abbr'];
				?>	
...
if (isset($_POST['cmdsearch'])){
	...
	<div class = "box-header with-border">			
		...
		SUBJECT:&nbsp;&nbsp;<?php echo strtoupper($subject)?>&nbsp;&nbsp;|&nbsp;&nbsp;PAPER:&nbsp;&nbsp;<?php echo $paper?><br><br>
		<h4><hr>
	</div>
	<div>
	<form role = "form" method = "POST" action = "marks_alevel.php">
		...
		while($student=mysql_fetch_array($students)){//This loop constructs the form for entering students marks
			...
			$query_mark = mysql_query("SELECT $sub_abbr FROM $marks_table WHERE sno='$student_number' AND term='$term' AND class='$class' 
						AND year='$Year' AND paper='$paper'",$con) or die('Kaakano'.mysql_error());
			while($std_mark = mysql_fetch_row($query_mark)){
				$mark = $std_mark[0];
			}
			if(mysql_num_rows($query_mark) == 0){
				$mark = '';
			}
			if($mark ==-1){$mark = '--';}
			if($mark ==-2){$mark = '';}
